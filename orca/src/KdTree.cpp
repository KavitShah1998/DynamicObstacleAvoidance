...
namespace RVO {
	...
	void KdTree::printKdTreeRecursive(ObstacleTreeNode* root) const{
		if(!root)
			return;

		std::cout << "Printing Root Node Data From kdTree.cpp: " << root-> obstacle -> point_ << "\n";
		
		printKdTreeRecursive(root->left);
		printKdTreeRecursive(root->right);
		return;
	}
}
