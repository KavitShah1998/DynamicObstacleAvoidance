...
namespace RVO {
	Agent::Agent(RVOSimulator *sim) : maxNeighbors_(0), maxSpeed_(0.0f), neighborDist_(0.0f), radius_(0.0f), sim_(sim), timeHorizon_(0.0f), timeHorizonObst_(0.0f), id_(0) { 
		agentStateSubscriber_ = nh_.subscribe("/my_robot/modelStates",10, &Agent::agentStateCallBackFunction, this);
	 }
	...
	void Agent::update()
	{
		velocity_ = newVelocity_; 			// pass this info to ROS cmd_vel
		
		//position_ += velocity_ * sim_->timeStep_;    // pause for some time or get this data from Gazebo_publisher
		//position_ = testSimPtr_->getRobotCurrentPosition();
		int count = 0;
		while(++count<4){
			ros::spinOnce();
		}
	...
	/*void Agent::getTestSimPtr(Test_Sim* ts){
		testSimPtr_ = ts;
	}*/
