...
namespace RVO {
	...
	private:
		 ...
		 *             data obtained by subscriber of Test_sim
		 */
		//void getTestSimPtr(Test_Sim*);
		...
		ros::NodeHandle nh_;
		ros::Subscriber agentStateSubscriber_;

		//Test_Sim* testSimPtr_{nullptr};
	 ...
	 * \param      directionOpt  True if the direction should be optimized.
	 * \param      result        A reference to the result of the linear program.
	 * \return     The number of the line it fails on, and the number of lines if successful.   %%% What if it fails on the last line?
	 */
